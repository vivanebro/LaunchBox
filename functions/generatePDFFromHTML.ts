...
Deno.serve(async (req) => {
    try {
        ...
        const { packages, colorScheme, photographerName, designComponent } = await req.json();
        ...
        const primary = hexToRgb(colorScheme.primary);
        const secondary = hexToRgb(colorScheme.secondary);
        ...
        doc.text(photographerName, pageWidth / 2, 20, { align: 'center' });
        ...
        return new Response(pdfBytes, {
            ...
            headers: {
                ...
                'Content-Disposition': `attachment; filename="${photographerName.replace(/\s+/g, '_')}_Packages.pdf"`
            }
        });
    } catch (error) {
...
function renderDesign1(doc, packages, primary, secondary, pageWidth, pageHeight) {
    ...
    packages.forEach((pkg, index) => {
        ...
        // Gradient header (simulated with multiple rectangles)
        for (let i = 0; i < 40; i++) {
            const ratio = i / 40;
        ...
        stats.forEach((stat, idx) => {
            ...
            doc.roundedRect(statX, currentY, statBoxWidth, 13, 2, 2, 'F');
            
            doc.setFontSize(9);
            doc.setFont(undefined, 'bold');
            doc.setTextColor(30, 30, 30);
            doc.text(stat.value, statX + statBoxWidth / 2, currentY + 6, { align: 'center' });
            
            doc.setFontSize(7);
            doc.setFont(undefined, 'normal');
            doc.setTextColor(120, 120, 120);
            doc.text(stat.label, statX + statBoxWidth / 2, currentY + 10, { align: 'center' });
        });
        ...
        // Features with proper checkmarks
        doc.setFontSize(8);
        doc.setFont(undefined, 'normal');
        doc.setTextColor(60, 60, 60);
        pkg.features.slice(0, 5).forEach((feature) => {
            if (currentY < startY + boxHeight - 18) {
                // Checkmark circle
                doc.setFillColor(primary.r, primary.g, primary.b);
                doc.circle(x + 8, currentY - 0.5, 2, 'F');
                
                // Checkmark symbol
                doc.setTextColor(255, 255, 255);
                doc.setFontSize(7);
                doc.setFont(undefined, 'bold');
                doc.text('✓', x + 8, currentY + 1, { align: 'center' });
                ...
                doc.text(lines[0].substring(0, 40), x + 13, currentY);
                currentY += 5.5;
            }
        });

        // Get Started button
        const buttonY = startY + boxHeight - 10;
        doc.setFillColor(primary.r, primary.g, primary.b);
        doc.roundedRect(x + 8, buttonY, boxWidth - 16, 8, 2, 2, 'F');
        
        doc.setFontSize(9);
        doc.setFont(undefined, 'bold');
        doc.setTextColor(255, 255, 255);
        doc.text('Get Started', x + boxWidth / 2, buttonY + 5.5, { align: 'center' });
    });
}
...
function renderDesign2(doc, packages, primary, secondary, pageWidth, pageHeight) {
    ...
    packages.forEach((pkg, index) => {
        ...
        if (pkg.isPopular) {
            doc.setFillColor(primary.r, primary.g, primary.b);
            doc.roundedRect(x + 10, currentY - 3, boxWidth - 20, 7, 2, 2, 'F');
            doc.setFontSize(7);
            doc.setFont(undefined, 'bold');
            doc.setTextColor(255, 255, 255);
            doc.text('★ BEST VALUE', x + boxWidth / 2, currentY + 1.5, { align: 'center' });
            currentY += 10;
        } else {
            currentY += 5;
        }
        ...
        // Stats inline with better background
        const statBoxWidth = 24;
        const statSpacing = 2;
        const statsStartX = x + (boxWidth - (statBoxWidth * 3 + statSpacing * 2)) / 2;
        ...
        pkg.features.slice(0, 5).forEach((feature) => {
            if (currentY < startY + boxHeight - 18) {
                ...
                doc.circle(x + 10, currentY - 0.5, 2, 'F');
                ...
                doc.text(lines[0].substring(0, 35), x + 15, currentY);
                currentY += 5.5;
            }
        });
...
function renderDesign3(doc, packages, primary, secondary, pageWidth, pageHeight) {
    ...
    packages.forEach((pkg, index) => {
        ...
        if (pkg.isPopular) {
            ...
            doc.roundedRect(x + boxWidth - 25, currentY - 6, 20, 7, 2, 2, 'F');
            doc.setGState(new doc.GState({ opacity: 1 }));
            
            doc.setFontSize(7);
            doc.setFont(undefined, 'bold');
            doc.text('Popular', x + boxWidth - 15, currentY - 1.5, { align: 'center' });
        }
...
function renderDesign4(doc, packages, primary, secondary, pageWidth, pageHeight) {
    ...
    packages.forEach((pkg, index) => {
        ...
        // Stats horizontal with background
        const statBoxWidth = 24;
        const statSpacing = 2;
        const statsStartX = x + (boxWidth - (statBoxWidth * 3 + statSpacing * 2)) / 2;
...
function renderDesign5(doc, packages, primary, secondary, pageWidth, pageHeight) {
    ...
    packages.forEach((pkg, index) => {
        ...
        doc.setFillColor(primary.r, primary.g, primary.b);
        doc.setGState(new doc.GState({ opacity: 0.12 }));
        doc.roundedRect(x + 10, currentY, boxWidth - 20, 26, 3, 3, 'F');
        doc.setGState(new doc.GState({ opacity: 1 }));

        doc.setFontSize(32);
        doc.setFont(undefined, 'bold');
        doc.setTextColor(primary.r, primary.g, primary.b);
        doc.text(`$${pkg.price}`, x + boxWidth / 2, currentY + 15, { align: 'center' });

        doc.setFontSize(7);
        doc.setFont(undefined, 'normal');
        doc.setTextColor(100, 100, 100);
        doc.text('starting at', x + boxWidth / 2, currentY + 21, { align: 'center' });
        ...
        // Features with alternating background and proper checkmarks
        doc.setFontSize(8);
        doc.setFont(undefined, 'normal');
        doc.setTextColor(60, 60, 60);
        pkg.features.slice(0, 5).forEach((feature, idx) => {
            if (currentY < startY + boxHeight - 18) {
                if (idx % 2 === 1) {
                    ...
                    doc.roundedRect(x + 6, currentY - 2.5, boxWidth - 12, 5.5, 1, 1, 'F');
                    doc.setGState(new doc.GState({ opacity: 1 }));
                }
                
                // Checkmark box
                doc.setFillColor(primary.r, primary.g, primary.b);
                doc.roundedRect(x + 10, currentY - 2, 4.5, 4.5, 1, 1, 'F');
                
                doc.setFontSize(6);
                doc.setTextColor(255, 255, 255);
                doc.setFont(undefined, 'bold');
                doc.text('✓', x + 12.25, currentY + 0.8, { align: 'center' });
                
                doc.setFontSize(8);
                doc.setFont(undefined, 'normal');
                doc.setTextColor(60, 60, 60);
                const lines = doc.splitTextToSize(feature, boxWidth - 24);
                doc.text(lines[0].substring(0, 35), x + 17, currentY);
                currentY += 5.5;
            }
        });
