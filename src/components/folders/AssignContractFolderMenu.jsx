...
import { assignContractToFolder, findOrCreateFolderByName } from '@/lib/folderAssignment';
import { toast } from '@/components/ui/use-toast';

export default function AssignContractFolderMenu({
  contractId,
  userId,
  initialFolderId,
  onFolderChange,
  ...
  const handleAssign = async (folderId) => {
    ...
    setLoading(true);
    try {
      const { folder, previousFolder } = await assignContractToFolder(contractId, folderId, userId);
  ...
  const handleCreateAndAssign = async () => {
    ...
    if (!name || !contractId || !userId) return;
    setLoading(true);
    try {
      const folder = await findOrCreateFolderByName(name, userId);
      const { previousFolder } = await assignContractToFolder(contractId, folder.id, userId);
    ...
    <DropdownMenu onOpenChange={(open) => open && loadFolders()}>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <span className="truncate">Folder: {currentName || 'None'}</span>
      ...
      <DropdownMenuContent align="start" className="w-72 p-0">
        ...
        <DropdownMenuItem onClick={() => handleAssign(null)} className="cursor-pointer text-gray-600">
          Remove from folder
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
